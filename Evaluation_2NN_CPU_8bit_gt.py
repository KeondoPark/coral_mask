...
def main():
    ...
    for filename in full_filenames:
        ...
        with open("./mAP/groundtruths/{}.txt".format(filenum), "r") as detection_file:
            
            lines = detection_file.readlines()

            for line in lines:
                obj = line.split(' ')
                gt_mask, xmin, ymin, xmax, ymax  = obj[0], int(float(obj[1])*width), int(float(obj[2])*height), int(float(obj[3])*width), int(float(obj[4])*height)
        ...
        #    break
                ...
                #with open("./mAP/2NN_CPU_8bit_detections_split/{}.txt".format(filenum), "a+") as file:
                #    file.write(label + ' ')
